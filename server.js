...
const dbFiles = {
  users: { f: 'users.json', init: [] },
  orders: { f: 'orders.json', init: [] },
  logs: { f: 'logs.json', init: [] },
  codes: { f: 'codes.json', init: [] },
  sessions: { f: 'sessions.json', init: [] },
  announcements: { f: 'announcements.json', init: [] },
  messages: { f: 'messages.json', init: [] },
  activity: { f: 'activity.json', init: [] }
};

for (const [key, conf] of Object.entries(dbFiles)) {
  const p = path.join(DATA_DIR, conf.f);
  if (!fs.existsSync(p)) fs.writeFileSync(p, key === 'settings' ? '' : '[]');
}

const readDB = (k) => JSON.parse(fs.readFileSync(path.join(DATA_DIR, dbFiles[k].f), 'utf-8'));
const writeDB = (k, d) => fs.writeFileSync(path.join(DATA_DIR, dbFiles[k].f), JSON.stringify(d, null, 2));

const logEvent = (type, message, meta = {}) => {
  const logs = readDB('logs');
  logs.unshift({ id: uuidv4().slice(0, 8), type, message, meta, timestamp: new Date().toISOString() });
...
const trackActivity = (userId, action, details = {}) => {
  ...
  acts.unshift({ id: uuidv4().slice(0, 8), userId, action, details, timestamp: new Date().toISOString() });
...
const DEFAULT_SETTINGS = {
  ...
  content: {
    ...
    heroSub: 'บริการขายโปรแกรมเมอร์ฟรีไฟ • เข้าใช้งานง่าย • ปลอดภัย 100% • อัปเดตทุกแพต�์',
    marquee: ['🔥 อัปเดต OB47 แล้ว', '⚡ เซิร์ฟเวอร์เ�ถียร 99.9%', '🎯 Aimbot • Wallhack • ESP • Anti-Ban', '💎 รองรับทุกดีไวซ์', '🚀 ติดตั้งง่ายใน 3 นาที']
  },
  packages: {
...
const getSettings = () => JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
const saveSettings = (s) => fs.writeFileSync(settingsFile, JSON.stringify(s, null, 2));
...
function ensureAdmin() {
  ...
  if (!admin) {
    ...
    const hash = crypto.createHash('sha256').update(ADMIN_ACCOUNT.password + JWT_SECRET).digest('hex');
    if (admin.password !== hash) { admin.password = hash; writeDB('users', users); }
    if (admin.role !== 'admin') { admin.role = 'admin'; writeDB('users', users); }
  }
  logEvent('admin', `Admin ${ADMIN_ACCOUNT.username} ready`);
}
ensureAdmin();
...
const auth = (req, res, next) => {
  ...
  try { req.user = jwt.verify(token, JWT_SECRET); next(); } catch { res.status(401).json({ error: 'Token หมดอายุ' }); }
};
const adminOnly = (req, res, next) => req.user.role === 'admin ? next() : res.status(403).json({ error: 'ต้องเป็นแอดมิน' });
...
app.get('/api/settings', (req, res) => {
  const s = getSettings();
  // hide sensitive
  delete s.payments.bank.account;
  res.json(s);
});
...
app.post('/api/login', (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ error: 'ไม่พบผู้ใ�้' });
...
app.post('/api/logout', (req, res) => {
  ...
  if (sid) {
    writeDB('sessions', readDB('sessions').filter(s => s.id !== sid));
  }
...
app.post('/api/orders/create', auth, (req, res) => {
  try {
    ...
    if (!method) return res.status(400).json({ error: 'เ�ือกวิธีชำระเงิน' });
...
app.post('/api/redeem', auth, (req, res) => {
  try {
    ...
    if (cd.type === 'credits') user.credits += cd.value;
...
app.get('/api/admin/orders', admin, (req, res) => {
  ...
  res.json({ orders: orders.slice(start, start + Number(limit)), total: orders.length });
});
...
app.get('/api/admin/users', admin, (req, res) => {
  ...
  res.json({ users: users.map(u => ({ ...u, password: undefined })), total: users.length });
});
...
app.delete('/api/admin/users/:id', admin, (req, res) => {
  const users = readDB('users').filter(u => u.id !== req.params.id && u.role !== 'admin');
  writeDB('users', users);
...
app.post('/api/admin/packages', admin, (req, res) => {
  ...
  const { id, name, price, oldPrice, days, image, tag, featured, enabled, order } = req.body;
  if (!id) return res.status(400).json({ error: 'ต้องมี id' });
  settings.packages[id] = { id, name, price, oldPrice, days, image, tag, featured: !!featured, enabled: enabled !== false, order: order || 99 };
...
app.post('/api/admin/packages/reorder', admin, (req, res) => {
  const settings = getSettings();
  const { order: ids } = req.body;
  ids.forEach((id, idx) => {
    if (settings.packages[id]) settings.packages[id].order = idx + 1;
  });
  saveSettings(settings);
  res.json({ success: true });
});
...
app.post('/api/admin/codes/create', admin, (req, res) => {
  ...
  const newCode = {
    id: uuidv4().slice(0, 8),
    code: (req.body.code || crypto.randomBytes(4).toString('hex').toUpperCase()).toUpperCase(),
    type: req.body.type || 'credits',
    value: req.body.value || 100,
    pkgId: req.body.pkgId || null,
    maxUse: req.body.maxUse || 1,
    used: 0,
    usedBy: [],
...
app.post('/api/admin/codes/bulk', admin, (req, res) => {
  ...
  writeDB('codes', codes);
  logEvent('bulk-code', `${count} codes`);
  res.json({ success: true, codes: created });
});

app.delete('/api/admin/codes/:id', admin, (req, res) => {
  writeDB('codes', readDB('codes').filter(c => c.id !== req.params.id));
  res.json({ success: true });
});
...
app.post('/api/admin/announcements', admin, (req, res) => {
  ...
  const ann = {
    ...
    type: req.body.type || 'info', // info/warning/success
    active: true,
    createdAt: new Date().toISOString(),
    createdBy: req.user.username
  };
...
app.delete('/api/admin/announcements/:id', admin, (req, res) => {
  writeDB('announcements', readDB('announcements').filter(a => a.id !== req.params.id));
  res.json({ success: true });
});
...
app.post('/api/admin/messages/send', admin, (req, res) => {
  ...
  logEvent('message', `To ${userId}: ${title}`);
...
app.post('/api/admin/system/clear-cache', admin, (req, res) => {
  const sessions = readDB('sessions').filter(s => s.expiresAt > Date.now());
  writeDB('sessions', sessions);
  res.json({ success: true });
});
...
app.get('/api/admin/system/info', admin, (req, res) => {
  const fs = require('fs');
  const files = Object.values(dbFiles).map(c => ({
    name: c.f,
    size: fs.statSync(path.join(DATA_DIR, c.f)).size
  }));
  res.json({
...
app.listen(PORT, () => console.log(`🔥 HIDDEN PRO on port ${PORT} | Admin: ${ADMIN_ACCOUNT.username}/${ADMIN_ACCOUNT.password}`));
